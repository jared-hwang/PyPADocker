#!/usr/bin/env python
#  $Id: mesh.py,v 1.1 2009/11/19 23:44:45 dave Exp $
#  -----------------------------------------------------------------
#
...
import unittest
#from Numeric import Float, Int, Int16, \
#   arange, array, product, rank, reshape, shape, zeros
from numpy.oldnumeric import Float, Int, Int16, \
   arange, array, product, rank, reshape, shape, zeros
from gist import *

# Int is Int32 (int)
# Float is Float64 (double)
# Int16 (short)
...
#     .. First, flatten the array to get the min and max
      ...
      _levels = array ( levs, Int, copy = 0 )

   print mesh.x
   print mesh.y
   print mesh.ireg
   print _levels
   plc_c ( _z, mesh.y, mesh.x, mesh.ireg, _levels, keywords )
...
#  .. Check the dimensions of mesh
   ...
   print mesh.y
   print mesh.ireg
   plf_c ( _z, mesh.y, mesh.x, mesh.ireg, keywords )
...
def plv ( vy, vx, y=None, x=None, ireg=None, mesh=None, **keywords ):
   ...
   """ 
   global savedMesh
   _vx = array ( vx, Float, copy=0 )
   _vy = array ( vy, Float, copy=0 )
...
#  .. Check the dimensions of mesh
   ...
   print mesh.y 
   print mesh.ireg 
   plv_c ( _vy, _vx, mesh.y, mesh.x, mesh.ireg, keywords )
