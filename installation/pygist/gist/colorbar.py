# $Id: colorbar.py,v 1.1 2009/11/19 23:44:45 dave Exp $
...
from numpy import *
from gist import *
from gist import gist as gistpy
from slice3 import *
...
def nice_levels (z, n = 8) :
   ...
   levs = cmin + (cmax - cmin)*arange(n)/(n-1)
...
def color_bar (minz, maxz, split = 0, ncol = None) :
   ...
   if type (minz) == type (maxz) == type (1) : # Do not change!!!
      plotval = reshape (arange (minz, maxz + 1, typecode = gistpy._bytetype),
         (maxz + 1 - minz, 1))
      pli (plotval, .60, ymin, .62, ymax) # draw bar
   elif not split :
      pli (reshape (arange(ncol)/(ncol-1.), (ncol, 1)),
         .60, ymin, .62, ymax) # draw bar
   else :
      pli(reshape(split_bytscl(arange(ncol)/(ncol-1.),0).astype(gistpy._bytetype),
          (ncol,1)),.60,ymin,.62,ymax) # draw bar
