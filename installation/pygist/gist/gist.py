#  $Id: gist.py,v 1.1 2009/11/19 23:44:45 dave Exp $
#  ---------------------------------------------------------------------
#
#  NAME:  gist.py
#
...
def span(zmin,zmax,nz,nn=None):
   result = numpy.linspace(zmin,zmax,nz)
   if nn is not None:
      result = result[:,newaxis]*ones(nn)
   return result
...
def spann (zmin, zmax, n = 8, fudge = 0, force = 0) :
   ...
   if (dz != 0.) :
      ...
      if nz > 1 :
         ...
         levs = zmin*dz + (zmax*dz - zmin*dz)*numpy.arange(nz)/(nz-1)
      else :
         if nz < 1 :
            if base < 1.5 :
               base = 5.0
...
def plfc (z, y, x, ireg, contours = 8, colors = None, region = 0,
   ...
   iregtemp = zeros(z.shape,'d')
   iregtemp[:-1,:-1] += (ireg[1:,1:] > 0)
   iregtemp[1:,:-1] += (ireg[1:,1:] > 0)
   iregtemp[:-1,1:] += (ireg[1:,1:] > 0)
   iregtemp[1:,1:] += (ireg[1:,1:] > 0)
   zmin = numpy.min(z)
   ...
   if isinstance(contours,int) :
      ...
      if scale == "lin" or scale is None :
          ...
          vc [1:n + 1] = vcmin + exp (arange (1, n + 1, 1, 'd') * \
             log (vcmax - vcmin) / (n + 1))
      elif scale == "normal" :
          zlin = ravel (z)
          lzlin = len (zlin)
