/* 
 *  $Id: gistCmodule_orig.c,v 1.1 2009/11/19 23:44:46 dave Exp $
 *  --------------------------------------------------------------------
...
static long _slice2_part (ArrayObject * xyzc, ArrayObject * keep,
...
static PyObject *contour (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  if ( !pyMsh.triangle )
     TRY (pyMsh.triangle = (PyArrayObject *) PyArray_SimpleNew (2, dims, PyArray_SHORT),
...
static PyObject *mfit (PyObject * self, PyObject * args)
{
 ...
 TRY (azcplot = (PyArrayObject *) PyArray_SimpleNew (2, dims, PyArray_DOUBLE),
...
static int set_def_reg (int nr, int nc)
{
  ...
  ne = nr * nc;
  newlen = ne + nc + 1;
  TRY (ra1 = (PyArrayObject *) PyArray_SimpleNew (1, &newlen, PyArray_LONG), 0);
  p1 = (int *) A_DATA (ra1);
...
static long set_reg (PyObject *op)
{
  ...
  /* printf("set_reg %d %d %d %d\n,isARRAY(op) , (A_NDIM(op) == 2) , A_TYPE(op) == PyArray_LONG , A_TYPE(op) == PyArray_INT); */
  ok = (isARRAY(op) && (A_NDIM(op) == 2) && (A_TYPE(op) == PyArray_LONG || A_TYPE(op) == PyArray_INT));
...
static long set_yx (PyObject *yop, PyObject *xop)
{
  ...
  pyMsh.y=(PyArrayObject *)PyArray_ContiguousFromObject(yop,PyArray_DOUBLE,2,2);
  if(!pyMsh.y) {
  ...
  pyMsh.x=(PyArrayObject *)PyArray_ContiguousFromObject(xop,PyArray_DOUBLE,2,2);
  if(!pyMsh.x) {
    Py_DECREF(pyMsh.y);
    pyMsh.y = NULL;
    if (!PyErr_Occurred()) PyErr_NoMemory ();
...
{ PyObject *dictionary;
  ...
  PyArrayObject *tickLen =
    (PyArrayObject *)PyArray_SimpleNew(1,dimensions,PyArray_DOUBLE);
  if (tickLen)
  { int i;
