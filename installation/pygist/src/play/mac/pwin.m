...
{ if (pw==nil) return; /* This window has already been destroyed. */
  void* ctx = pw->ctx;
         ...
         unsigned long bg, int hints, void *ctx)
{
  ...
  /* window retains a reference to view */
  [view release];
...
p_destroy(p_win *pw)
{
  ...
  { [view releaseGState];
    ...
    /* window contains the only reference to view; no need to release view */
    [view switchoff];
  }
