...
static int GetCommand(char *token)
{
  ...
  else if (strcmp(token, "F")==0) now= "10f";
  else if (strcmp(token, "b")==0) now= "1b";
  else if (strcmp(token, "B")==0) now= "10b";
  else if (strcmp(token, "g")==0) now= "1g";
  else if (strcmp(token, "G")==0) now= "100000g";
...
static void HandleOther(Engine *engine, int k, int md)
{
  ...
  else if (k=='f' || (k=='+' && (md&P_KEYPAD))) go= 1;
  else if (k=='F') {if (xPrefix==0) xPrefix=1; xPrefix*=xFrameStep; go= 1;}
  else if (k=='b' || (k=='-' && (md&P_KEYPAD))) go= 2;
