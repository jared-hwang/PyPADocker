 ...
 *
 *  CHANGES:  See the ChangeLog file for a description of changes.
 *
 *  --------------------------------------------------------------------
 */
...
#define ERRSS(s) ((PyObject *)(PyErr_SetString(GistError,s),(long)0))
...
#define TRY(e, m) do{if(!(e)){clearArrayList();clearFreeList(0);\
               clearMemList();return (PyErr_Occurred() ? (long)0 : m);}} while(0)
...
#define GET_ARR(ap,op,type,dim,cast) \
  ...
  (cast)PyErr_NoMemory ())
/* Build a new array */
#define NEW_ARR_FROMARRAY(ap,n,dims,type,cast) \
  TRY(addToArrayList((PyObject *)(ap=\
  (PyArrayObject *)PyArray_SimpleNew(n,dims,type))), \
  (cast)PyErr_NoMemory ())
...
/* freeList = GArrayObjects (list 0 is slice2 list, list1 is _slice2_part) */
...
static GArrayObject * freeList [MAX_NO_LISTS] [MAX_LIST_SIZE];
...
/*****************************************************************/
/*           GArrayObject creation                                */
/* allocateArray creates a complete new GArrayObject including a  */
/* newly malloc'ed data area set to zeros.                       */
/* arrayFromPointer creates a new GArrayObject pointing to a data */
...
static int addToFreeList (GArrayObject * x, int n);
static void freeArray (GArrayObject * a, int n);
static void removeFromFreeList (GArrayObject * x, int n);
...
static GArrayObject * allocateArray (npy_intp size, char tc, int nlist) {
   ...
   TRY (res = calloc (1, sizeof (GArrayObject)), 
      ...
      switch (tc) {
         case 'i' :
            if ( !(res->data = (void *) calloc (size, sizeof (int)))) {
               free (res) ;
               return (GArrayObject *) PyErr_NoMemory ();
               }
            break;
         case 'd' :
   ...
   if (addToFreeList (res, nlist) != 0) {
      freeArray (res, nlist);
      res = (GArrayObject *) NULL ;
      }
   return res;
   }

static GArrayObject * copyArray (GArrayObject * a) {
   /* returns a copy of the argument array. Does not put it on */
   /* the free list.                                           */
   GArrayObject * res;
   int size;
   if ( a == (GArrayObject *) NULL || a->size <= 0) {
      return (GArrayObject *) NULL;
      }
   switch (a->typecode) {
      case 'b':
   ...
   TRY (memcpy (res->data, a->data, a->size * size),
      (GArrayObject *) ERRSS ("memcpy failed in copyArray."));
   res->size = a->size;
 ...
 * Routines to create GArrayObjects may return a NULL object, which
 ...
 * conventions.
 * Every time an GArrayObject is successfully created and returned
...
static void removeArrayOnly (GArrayObject * x, int n) {
...
static long setkw_boolean (PyObject * v, int *t, char *kw);
static long setkw_color (PyObject * v, unsigned long *t, char *kw);
...
static long setkw_xinteger (PyObject * v, int *t, char *kw);
static long setvu_mesh( PyObject *args, PyObject **vop, 
  PyObject **uop, char *errstr);
static long setz_mesh( PyObject *args, PyObject **zop, 
 ...
 *
 *      iMax == nc == PyArray_DIM(array, 1)
 *      jMax == nr == PyArray_DIM(array, 0)
 */
static struct {
...
#ifdef INCLUDE_EXTRA_GISTCODE 
static GArrayObject * Add1 ( GArrayObject * i, int freei, int n) {
   /* add 1 to an integer array i and return the value */
   GArrayObject * res ;
   int * isrc ,
       * itar;
   if (i == (GArrayObject *) NULL)
      return (GArrayObject *) NULL;
   if (freei >= 0 && freei == n) {
...
static GArrayObject * concatenate (GArrayObject * x, GArrayObject * y,
   ...
   int datasize;
   Uchar tc;
   if (x == (GArrayObject *) NULL && y == (GArrayObject *) NULL) {
      return (GArrayObject *) NULL;
      }
   tc = (y == (GArrayObject *) NULL) ? x->typecode : y->typecode;
   switch (tc) {
      case 'b':
   ...
   if (y == (GArrayObject *) NULL) {
      TRY (result = allocateArray (x->size, x->typecode, n), 
         (GArrayObject *) PyErr_NoMemory ());
      TRY (memcpy (result->data, x->data, x->size * datasize),
         (GArrayObject *) ERRSS ("memcpy failed in concatenate."));
      if (freex >= 0)
         freeArray (x, freex);
      return result;
      }
   if (x == (GArrayObject *) NULL) {
...
static GArrayObject * cumsum (GArrayObject * i, int freei, int n) {
...
static GArrayObject * equal (GArrayObject * i, GArrayObject * j,
   ...
   GArrayObject * res;
   Uchar * tar;
   if (i == (GArrayObject *)NULL || j == (GArrayObject *)NULL ||
      i->size != j->size)
      return (GArrayObject *) NULL;
...
static GArrayObject * greater (GArrayObject * d, double v, int freed, int n) {
...
static GArrayObject * greater_equal (GArrayObject * d, double v,
   int freed, int n) {
   /* compare an array object of type 'd' with a double v; return an
    * array  of Uchar the same size with 1's where the compare succeeds. */
   double * dr ;
...
static GArrayObject * less (GArrayObject * i, GArrayObject * j,
   int freei, int freej, int n) {
   int * src1,
       * src2;
   GArrayObject * res;
...
static GArrayObject * logical_and (GArrayObject * a, GArrayObject * b,
   ...
   Uchar * src1,
         * src2,
         * tar;
   if (a == (GArrayObject *) NULL || b == (GArrayObject *) NULL ||
...
static GArrayObject * logical_not (GArrayObject * b, int freeb, int n) {
   /* Take the logical not of an int or Uchar array and return Uchar */
   GArrayObject * res;
   Uchar * usrc,
         * tar;
   int * isrc;
   if (b == (GArrayObject *) NULL)
      return (GArrayObject *) NULL;
   if (freeb >= 0 && freeb == n) {
      /* do it in place */
      if (b->typecode == 'b') {
...
#ifdef INCLUDE_EXTRA_GISTCODE
static GArrayObject * not_equal (GArrayObject * i, int j, int freei, int n) {
...
static GArrayObject * SimpleHist (GArrayObject * i, int freei, int n) {
...
static GArrayObject * take (GArrayObject * a, GArrayObject * i, int nelts,
   ...
   Uchar * utar,
         * usrc;
   int j;
   if (a == (GArrayObject *) NULL || nelts <= 0 ||
       i == (GArrayObject *) NULL || i->size == 0) {
      return (GArrayObject *) NULL ;
      }
...
#ifdef INCLUDE_EXTRA_GISTCODE
static GArrayObject * WeightedHist (GArrayObject *i, GArrayObject *w,
   ...
   Uchar * wgt;
   int max;
   if (i == (GArrayObject *) NULL || w == (GArrayObject *) NULL ||
      i->size > w->size)
      return (GArrayObject *) NULL;
   for (src = (int *) (i->data), max = * (src ++);
        src < (int *) (i->data) + i->size; src++)
...
static long _slice2_part (GArrayObject * xyzc, GArrayObject * keep,
   GArrayObject * next, GArrayObject * dp, GArrayObject * prev,
   GArrayObject * last, GArrayObject * valc, GArrayObject ** xyzc_new,
   GArrayObject ** nvertc, GArrayObject ** valc_new, int freexyzc, 
   ...
   GArrayObject * list = (GArrayObject *) NULL,
               * list0 = (GArrayObject *) NULL,
               * list1 = (GArrayObject *) NULL,
               * mask = (GArrayObject *) NULL,
               * ndxs = (GArrayObject *) NULL,
               * valc0 = (GArrayObject *) NULL,
               * valc1 = (GArrayObject *) NULL,
               * xold = (GArrayObject *) NULL,
               * xyz0 = (GArrayObject *) NULL,
               * xyz1 = (GArrayObject *) NULL;
   Uchar * keepd,
         * maskd;
   int * nextd=0,
       * prevd=0,
   ...
   /* i = dmalloc_verify (0); */
   * xyzc_new = (GArrayObject *) NULL;
   * nvertc = (GArrayObject *) NULL;
   if (valc) {
      /* Don't touch the following if no values are coming in!! */
      * valc_new = (GArrayObject *) NULL;
      if (atype == 'd')
   ...
   TRY (mask = allocateArray (keep->size, 'b', 1), (long) NULL);
   ...
   TRY (list = allocateArray (mask->size, 'i', 1), (long) PyErr_NoMemory ());
   listd = (int *) (list->data);
   ...
   TRY (* xyzc_new = allocateArray (3 * xold->size, 'd', 0), (long) NULL);
   xyzc_newd = (double *) ( (* xyzc_new)->data);
   if (valc != (GArrayObject *)NULL) {
      TRY (* valc_new = allocateArray (xold->size, atype, 0), (long) NULL);
      if (atype == 'd')
...
static char *CheckDefaultWindow(void)
{
  ...
  if (curPlotter<0) {
    ...
    if (!ghDevices[0].drawing)
      return ("failed to create drawing -- Gist work.gs style sheet missing. Set GISTPATH to be path where the file is located.");
...
static long GetTypeface (const char *s, int *f)
{
  ...
  while (*s) {
    if (*s == 'B' && !(face & T_BOLD))
      ...
      return (long) ERRSS ( "illegal font keyword suffix -- B is bold, I is italic");
    }
    s++;
  }
  *f = face;
...
static PyObject *contour (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  int n,
      ...
      region = 0;
  npy_intp dims [2],            /* dimensions of mesh                  */
  ...
  dims [0] = PyArray_DIM (zap, 0);
  dims [1] = PyArray_DIM (zap, 1);
  if (dims [0] != PyArray_DIM (pyMsh.y, 0) || dims [1] != PyArray_DIM (pyMsh.y, 1)) {
     return ERRSS ("z array must have same dimensions as mesh in contour.");
  }
  /* Provide a triangle if none supplied */
  if ( !pyMsh.triangle )
     TRY (pyMsh.triangle = (PyArrayObject *) PyArray_SimpleNew (2, dims, NPY_SHORT),
...
static long SetHCPDefault (void)
{
  ...
  return (long) ERRSS ( "you appear to have Aa00 through Zz00 hcp files -- clean up");

got1:
  if (!hcpPSdefault)
    hcpDefault = GpCGMEngine ("Pygist default", 0, hcpDump,
                            SetHCPname (-1, hcpName,0));
  else
    hcpDefault = GpPSEngine ("Pygist default", 0, hcpDump,
                            SetHCPname (-1, hcpName,0));

  if (!hcpDefault)  {
    return (long) ERRSS ("failed to create default hcp file");
  }
 ...
 */
static long build_kwt (PyObject *kd, char *kwlist[], PyObject * kwt[])
{
  int i, n, nkw_set = 0;
  char *kw;
...
static PyObject *bytscl (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  TRY (addToArrayList((PyObject *)(zap = (PyArrayObject *)
      PyArray_ContiguousFromObject (zop, NPY_DOUBLE, 1, 0))),
...
static void get_mesh(GaQuadMesh *m)
{
  m->iMax = PyArray_DIM (pyMsh.y, 1);
  m->jMax = PyArray_DIM (pyMsh.y, 0);
...
static PyObject *hcp_file (PyObject * self, PyObject * args, PyObject *kd)
{
  ...
  if (hcp) {
    ...
    if (engine) {
      hcpDefault = 0;
      GpKillEngine (engine);
      SetHCPname (-1, (char *) 0,0);
      engine = 0;
    }
    if (len > 3 && strcmp (&hcp[len - 3], ".ps") == 0) {
      engine = GpPSEngine ("Pygist default", 0, hcpDump, SetHCPname(-1, hcp,0));
      if (!engine) {
        return ERRSS ("failed to create PostScript file");
      }
    } else if (len > 0) {
      engine = GpCGMEngine ("Pygist default", 0, hcpDump, SetHCPname(-1, hcp,0));
      if (!engine) {
...
static PyObject *limits (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  long nkw;
  int old_flags, new_flags = 0, changed = 0, j;
  int square=0, nice=0, g_restrict=0;
...
static PyObject *mesh_loc (PyObject * self, PyObject * args)
{
  ...
  if (isARRAY (y0op)) {
    TRY (addToArrayList((PyObject *)(y0ap = (PyArrayObject *)
        PyArray_ContiguousFromObject (y0op, NPY_DOUBLE, 1, 0))),
        (PyObject *)PyErr_NoMemory ());
    n = A_SIZE ( y0ap );
    TRY (addToArrayList((PyObject *)(x0ap = (PyArrayObject *)
        PyArray_ContiguousFromObject (x0op, NPY_DOUBLE, 1, 0))),
        (PyObject *)PyErr_NoMemory ());
    if (n != A_SIZE ( x0ap )) {
      clearArrayList();
...
static PyObject *mfit (PyObject * self, PyObject * args)
{
 ...
 nzcplot = PyArray_DIM (aalpha, 0);
 nxcplot = PyArray_DIM (axcplot, 0);
 dims [0] = nxcplot;
 nycplot = PyArray_DIM (aycplot, 0);
 dims [1] = nycplot;
 x = (double *) A_DATA (ax);
 y = (double *) A_DATA (ay);
 ...
 TRY (azcplot = (PyArrayObject *) PyArray_SimpleNew (2, dims, NPY_DOUBLE),
...
static PyObject *plc (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  GET_ARR (zap, zop, NPY_DOUBLE, 2, PyObject *);
  jMax = PyArray_DIM(zap, 0);
  iMax = PyArray_DIM(zap, 1);
  if (PyArray_DIM (pyMsh.y, 0) != jMax || PyArray_DIM (pyMsh.y, 1) != iMax) {
    clearArrayList ();
...
static PyObject *pldj (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  for (i=0; i<4; i++)
    TRY (addToArrayList ((PyObject *)(ap[i] = (PyArrayObject *)
        PyArray_ContiguousFromObject (op[i], NPY_DOUBLE, 1, 0))),
        (PyObject *)PyErr_NoMemory ());
...
static PyObject *plf (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  if (isARRAY(zop) && (A_TYPE(zop) == Py_GpColor)) {
    ...
    if (isARRAY(zop) && (A_TYPE(zop) == NPY_DOUBLE)) {
      GET_ARR (zap, zop, NPY_DOUBLE, 2, PyObject *);
      z = (double *) A_DATA (zap);
    } else {
      z = 0;
...
static PyObject *plfp (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  } else if (isARRAY(zop) && (A_TYPE(zop) == NPY_DOUBLE)) {
...
static PyObject *plv (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  jMax = (PyArray_DIM(vap, 0) == PyArray_DIM(uap, 0)) ? PyArray_DIM(vap, 0) : 0;
  iMax = (PyArray_DIM(vap, 1) == PyArray_DIM(uap, 1)) ? PyArray_DIM(vap, 1) : 0;
  if (PyArray_DIM (pyMsh.y, 0) != jMax || PyArray_DIM (pyMsh.y, 1) != iMax ) {
    clearArrayList ();
...
static long set_def_reg (long nr, long nc)
{
  ...
  ne = nr * nc;
  newlen = ne + nc + 1;
  TRY (ra1 = (PyArrayObject *) PyArray_SimpleNew (1, &newlen, NPY_LONG), 0);
  p1 = (int *) A_DATA (ra1);
...
static long set_reg (PyObject *op)
{
  ...
  /* Fill in the data part of the new region array. */
  for (i = 0;      i <= nc; i++)    p1[i]      = 0;
  for (i = nc+1;   i <  ne; i++)    p1[i]      = (int)p2[i];
  for (i = 0;      i <  nc; i++)    p1[ne + i] = 0;
  for (i = 2*nc;   i <  ne; i+=nc)  p1[i]      = 0;
...
static long set_tri (PyObject *top)
{
  ...
  nr = PyArray_DIM (pyMsh.y, 0);
  nc = PyArray_DIM (pyMsh.y, 1);

  Py_XDECREF (pyMsh.triangle);
  GET_ARR (pyMsh.triangle, top, NPY_SHORT, 2, long);

  if (PyArray_DIM (pyMsh.triangle, 0) != nr || PyArray_DIM (pyMsh.triangle, 1) != nc) {
    removeFromArrayList ((PyObject *)pyMsh.triangle);
    return (long) ERRSS ("triangle array must match shape of (y, x).");
  }
  array_list_length = 0;
  return 1;
}
...
static long set_yx (PyObject *yop, PyObject *xop)
{
  ...
  pyMsh.y=(PyArrayObject *)PyArray_ContiguousFromObject(yop,NPY_DOUBLE,2,2);
  if(!pyMsh.y) {
  ...
  if (nr < 2 || nc < 2) {
    Py_DECREF(pyMsh.y);
    pyMsh.y = NULL;
    return (long) ERRSS ("(y, x) arrays must be at least 2X2");
  }

  pyMsh.x=(PyArrayObject *)PyArray_ContiguousFromObject(xop,NPY_DOUBLE,2,2);
  if(!pyMsh.x) {
    Py_DECREF(pyMsh.y);
    pyMsh.y = NULL;
    if (!PyErr_Occurred()) PyErr_NoMemory ();
  ...
  if (PyArray_DIM (pyMsh.x, 0) != nr || PyArray_DIM (pyMsh.x, 1) != nc) {
    Py_DECREF(pyMsh.y);
    Py_DECREF(pyMsh.x);
    pyMsh.y = NULL;
    pyMsh.x = NULL;
    return (long) ERRSS ("x array must match shape of y");
  }
...
static long setkw_justify (PyObject * v, int *t, char *kw)
{
  ...
  if (PyString_Check (v)) {
    ...
    if (*s == 'N') {
      ...
      return (long) ERRSS ("unrecognized justify keyword");
    }
   ...
   [nverts, xyzverts, values, nvertb, xyzvertb, valueb]            */
{
 ...
 GArrayObject * rnverts = (GArrayObject *) NULL,
             * rxyzverts = (GArrayObject *) NULL,
             * rvalues = (GArrayObject *) NULL,
             * rnvertb = (GArrayObject *) NULL,
             * rxyzvertb = (GArrayObject *) NULL,
             * rvalueb = (GArrayObject *) NULL,
             * dp = (GArrayObject *) NULL,
             * ndp = (GArrayObject *) NULL,
             * nvertc = (GArrayObject *) NULL,
             * xyzc = (GArrayObject *) NULL,
             * valuec = (GArrayObject *) NULL,
             /* values returned from _slice2_part */
             * rnvertc,
             * rxyzc,
             * rvaluec = (GArrayObject *) NULL,
             * nvertc0 = (GArrayObject *) NULL,
             * xyzc0 = (GArrayObject *) NULL,
             * valuec0 = (GArrayObject *) NULL,
             * keep = (GArrayObject *) NULL,
             * prev = (GArrayObject *) NULL,
             * next = (GArrayObject *) NULL,
             * last = (GArrayObject *) NULL,
             * nkeep,
             * nkeep2,
             * mask2,
             * list = (GArrayObject *) NULL,
             * listc = (GArrayObject *) NULL;
 double * planed=0,
        * xyzvertsd=0,
        * valuesd = (double *) NULL,
        * rxyzvertsd=0,
 ...
 GET_ARR (anverts, onverts, NPY_LONG, 1, PyObject *);
 GET_ARR (axyzverts, oxyzverts, NPY_DOUBLE, 2, PyObject *);
 if (isARRAY (ovalues)) {
    if (A_TYPE (ovalues) == NPY_DOUBLE) {
       GET_ARR (avalues, ovalues, NPY_DOUBLE, 1, PyObject *);
...
/* freeArray (nkeep, 0); Need nkeep and nverts instead of mask0 */
 ...
 if (list0_length < A_SIZE (anverts) ) {
    ...
    /* We've given the data pointers of these two or three arrays to others, so they
       no longer own their data. Clear the NPY_OWN_DATA flags, so that the
 ...
 if (list1_length == 0 && listc_length == 0) {
    if (rnverts) {
       RET_ARR (ornverts, 1, & (rnverts->size), NPY_LONG, (char *) rnvertsd,
          PyObject *);
       SET_OWN (ornverts);
       }
    else {
    ...
    if (rxyzverts) {
       ...
       xdims [1] = 3;
       RET_ARR (orxyzverts, 2, xdims, NPY_DOUBLE, (char *) rxyzvertsd,
          PyObject *);
       SET_OWN (orxyzverts);
       }
    else {
    ...
    if (rxyzvertb == NULL) {
       ...
       /* Build the rest */
       RET_ARR (ornvertb, 1, & (rnvertb->size), NPY_LONG, (char *) rnvertbd,
          PyObject *);
       SET_OWN (ornvertb);
       xdims [0] = rxyzvertb->size / 3;
       xdims [1] = 3;
       RET_ARR (orxyzvertb, 2, xdims, NPY_DOUBLE, (char *) rxyzvertbd,
          PyObject *);
       SET_OWN (orxyzvertb);
       if (avalues) {
          if (atype == 'd') {
 ...
 if (list1_length != 0) {
    ...
    if (avalues && node == 1) {
       ...
       TRY (_slice2_part (xyzc, keep, next, dp, prev, last,
          (GArrayObject *) NULL, & rxyzc, & rnvertc, & rvaluec, FREE0, SAVE, atype), 
          PyErr_NoMemory());
       if (valuec) {
 ...
 if (_slice2x || list1_length == 0) {
    ...
    if (avalues && node == 1) {
       ...
       TRY (_slice2_part (xyzc0, keep, next, dp, prev, last,
          (GArrayObject *) NULL, & rxyzc, & rnvertc, & rvaluec,
          FREE0, FREE0, atype), 
          PyErr_NoMemory());
       if (valuec0) {
    ...
    if (rnvertb == (GArrayObject *) NULL) {
       ...
       if (avalues) {
          ...
          Py_INCREF (Py_None);
          rvalueb = (GArrayObject *) Py_None;
          }
       rxyzvertb = rxyzc;
       rxyzvertbd = (double *) rxyzvertb->data;
       }
    else if (rnvertc != (GArrayObject *) NULL) {
 ...
 if (rvalues != (GArrayObject *) Py_None) {
    ...
    if (atype == 'd') {
       ...
       RET_ARR (orvalues, 1, & (rvalues->size), atype, (char *) rvaluesc,
          PyObject *);
       }
    SET_OWN (orvalues);
    }
 ...
 if (rxyzvertb == NULL) {
    ...
    RET_ARR (orxyzvertb, 2, xdims, NPY_DOUBLE, (char *) rxyzvertbd,
       PyObject *);
    SET_OWN (orxyzvertb);
    if (rvalueb != (GArrayObject *) Py_None) {
       if (atype == 'd') {
          RET_ARR (orvalueb, 1, & (rvalueb->size), atype,
             (char *) rvaluebd, PyObject *);
          }
...
static PyObject *window (PyObject * self, PyObject * args, PyObject * kd)
{
  ...
  if (kwt[3]) {
    ...
    if (hcp && hcp[0]) {
      ...
      if (len > 3 && strcmp (&hcp[len - 3], ".ps") == 0) {
        engine = GpPSEngine (windowNames[n], 0, hcpDump, SetHCPname (n, hcp,1));
        if (!engine)  {
          return ERRSS ("failed to create PostScript file");
        }
      } 
      else {
        engine = GpCGMEngine (windowNames[n], 0, hcpDump, SetHCPname (n, hcp,1));
        if (!engine)  {
          return ERRSS ("failed to create binary CGM file");
        }
      }
  ...
  if (!ghDevices[n].display && !ghDevices[n].hcp) {
    ...
    if (kwt[6]) {
      ...
      if (style != defaultStyle) SAFE_FREE(style);
...
/*
   ...
   $Id: style.c,v 1.1 2009/11/19 23:44:48 dave Exp $
 */
...
{ PyObject *dictionary;
  npy_intp dimensions[] = {TICK_LEVELS};
  ...
  PyArrayObject *tickLen =
    (PyArrayObject *)PyArray_SimpleNew(1,dimensions,NPY_DOUBLE);
  if (tickLen)
  { int i;
...
*/
...

  {
     ...
     */
    if (!PyOS_InputHook) PyOS_InputHook = p_wait_stdin;
